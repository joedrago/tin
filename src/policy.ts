...
export function describeTinjs(commands: string[]): string[] {
	...
	return [
		...
		`  - Added to it: \`read(path)\` and \`readBytes(path)\` for any path on this machine, \`readBytes(path, offset, length)\` for a slice of a large or binary file without reading the rest, \`stat(path)\` for \`{ size, mtime, isDirectory, isFile }\` before deciding how to read something, \`lines(path)\`, \`print(...)\`, \`console.log/error\`, \`inspect(value)\`, \`args\` for the arguments after the script, and \`exit(code)\`.`,
